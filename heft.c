...
void calculate_EST_EFT(int task,int processor,struct TaskProcessor *EST_EFT)
{
    ...
    for(i=0; i<noslots; i++)
    {
        ...
        {
            if((machineFreeTime[i][0] + computation_costs[processor][task]) <= machineFreeTime[i][1])
            {
                EST_EFT->AST=machineFreeTime[i][0];
                EST_EFT->AFT=machineFreeTime[i][0] + computation_costs[processor][task];
                return;
            }
        }
        if(EST>=machineFreeTime[i][0])
        {
            if((EST + computation_costs[processor][task]) <= machineFreeTime[i][1])
            {
                EST_EFT->AFT=EST_EFT->AST + computation_costs[processor][task];
                return;
            }
        }
    }
...
void make_schedule()
{
    ...
    for(i=0; i<no_tasks; i++)
    {
        ...
        if(isEntryTask(task))
        {
            for(j=0; j<no_machines; j++)
            {
                if(minCost>computation_costs[j][task])
                {
                    minCost=computation_costs[j][task];
                    processor=j;
                }
            }
...
int main()
{
    ...
    computation_costs=(double**)calloc(no_machines,sizeof(double*));

    for(int i=0; i<no_machines; i++)
        computation_costs[i]=(double*)calloc(no_tasks,sizeof(double));
